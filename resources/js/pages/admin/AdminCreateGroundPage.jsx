...
export default function AdminCreateGroundPage() {
	...
	const handleSubmit = async (values) => {
		...
		try {
			...
			const response = await axios.post('/admin/grounds/create', submitData, {
				headers: {
			...
			if (response.data.status === 'success') {
				message.success(response.data.message || 'Ground created successfully!');
			...
			if (err.response?.status === 422) {
				...
				message.error(err.response?.data?.message || 'Failed to create ground');
			}
		} finally {
			setSubmitting(false);
		}
		...
		<AntDesignAdminLayout>
			<Space direction="vertical" size="large" style={{ width: '100%' }}>
				<Space>
					...
					<Title level={2} style={{ margin: 0 }}>Create New Ground</Title>
				</Space>
				...
				<Form
					...
					initialValues={{
						...
						is_new: false,
						is_featured: false,
						features: [{ feature_name: '', feature_type: 'facility', feature_status: 'active' }],
						slots: [{ slot_name: '', slot_type: 'morning', slot_status: 'active', price_per_slot: 0 }],
					}}
